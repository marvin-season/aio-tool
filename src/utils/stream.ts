...
export async function* SSEMessageGenerator<T extends string | ArrayBuffer>(
...
export function createMockStream<T>(
  ...
  options: {
    regx?: RegExp;
    chunk_transform?: (chunk: string, id: number) => T;
  }
) {
  if (!input) {
  ...
  const { regx = /\s+/, chunk_transform = (chunk) => chunk } = options;

  const id = Date.now();
  return new ReadableStream<T>({
    async start(controller) {
      for (const chunk of input.split(regx)) {
        controller.enqueue(chunk_transform(chunk, id) as T);
      }
 ...
 * @beta
 */
export function parseThinkContent(input: string): {
  content: string;
